function view_cast(show_intro) {
  ...
  var redraw_tab = function (tab_id, $tab) {
    ...
    if (tab_info.characters.length > 0) {
      ...
      $tab.append($("<div/>").html(tab_info.name)).append("Il n'y a actuellement aucun PNJ sur cet onglet.");
    }
...
function view_generator() {
  ...
  $template.find('#generate_mob').on('click', function () {
    ...
    $popup.dialog({
      autoOpen: false,
      modal: true,
      title: 'Ajouter les PNJ à un onglet spécifique',
      width: 500,
      buttons: [
        {
          text: "Annuler",
...
function setup_controls() {
  ...
  if (!localStorage) {
    // TODO prettyify
    alert('Votre navigateur est trop ancien pour exécuter ce code. Veuillez mettre à jour votre navigateur.');
  }
  ...
  $('.top_bar .numbers button').on('click', function () {
    ...
    addition += timestamp.toLocaleTimeString() + '; lance ' + dice + 'D6';
    ...
    addition += (results.hits === 1) ? ' succés' : ' succés';
